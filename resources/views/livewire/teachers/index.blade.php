<div>
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('teachers.create') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Add Teacher</a>
    </div>
    <div class="mb-4">
        <input type="text" wire:model.live="search" placeholder="Search teachers..." class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
    </div>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($teachers as $teacher)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">{{ $teacher->school->name }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{ $teacher->subject }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <a href="{{ route('teachers.edit', $teacher->id) }}" class="text-blue-600 hover:text-blue-900 mr-3">Edit</a>
                            <button wire:click="confirmDelete({{ $teacher->id }})" x-data @click="$dispatch('open-modal', 'confirm-delete')" class="text-red-600 hover:text-red-900">Delete</button>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="5" class="px-6 py-4 text-center text-gray-500">No teachers found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    ...
                    <p class="mt-2 text-sm text-gray-500">Are you sure you want to delete this teacher?</p>
                </div>
